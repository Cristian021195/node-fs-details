...
(async () => {
 ...
 //RECORDAR, AL LEER O MODIFICAR UN ARCHIVO, SIEMPRE HAY QUE ABRIRLO PRIMERO CON OPEN O FUNCIONES DE NODE QUE INTERNAMENTE USEN OPEN
  const createFile = async (path) => {
    try {
      // we want to check whether or not we already have that file
      const existingFileHandle = await fs.open(path, "r");// or appendFile func
  ...
  const deleteFile = async (path) => {
    try {
      await fs.unlink(path);// or rm - rmDir
